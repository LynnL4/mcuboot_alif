                                 ...
                                 uint16_t status)
{
    ...
    {
        BOOT_LOG_ERR("Advertising completion callback failed, error: %u", status);
        return;
    }
    ...
    case GAPM_ACTV_CREATE_LE_ADV:
    {
        ...
        if (rc != GAP_ERR_NO_ERROR)
        {
            BOOT_LOG_ERR("Failed to create advertisement data, error: %u", rc);
            return;
        }
    }
    break;
    ...
    case GAPM_ACTV_SET_ADV_DATA:
    {
        ...
        if (rc != GAP_ERR_NO_ERROR)
        {
            BOOT_LOG_ERR("Failed to set scan data, error: %u", rc);
            return;
        }
    }
    break;
    ...
    case GAPM_ACTV_SET_SCAN_RSP_DATA:
    {
        ...
        if (rc != GAP_ERR_NO_ERROR)
        {
            BOOT_LOG_ERR("Failed to start advertising, error: %u", rc);
            return;
        }
    }
    break;
    ...
    case GAPM_ACTV_START:
    {
        ...
        BOOT_LOG_INF("Advertising has been started, address: %02X:%02X:%02X:%02X:%02X:%02X",
...
{
    BOOT_LOG_INF("Advertising has been stopped");
}
                                 ...
                                 const gapc_le_con_param_t *p_con_params, uint8_t clk_accuracy)
{
    ...
    if (rc != GAP_ERR_NO_ERROR)
    {
        BOOT_LOG_ERR("Failed to accept incoming connection, error: %u", rc);
        return;
    }

    BOOT_LOG_INF("New client connection from %02X:%02X:%02X:%02X:%02X:%02X (conidx: %u)",
...
static void on_disconnection(uint8_t conidx, uint32_t metainfo, uint16_t reason)
{
    ...
    BOOT_LOG_INF("Client disconnected (conidx: %u), restating advertising", conidx);
    ...
    if (rc != GAP_ERR_NO_ERROR)
    {
        BOOT_LOG_ERR("Failed to restart advertising, error: %u", rc);
        return;
    }
}
...
static void on_appearance_get(uint8_t conidx, uint32_t metainfo, uint16_t token)
{
    ...
    if (rc != GAP_ERR_NO_ERROR)
    {
        BOOT_LOG_ERR("Failed to send appearance error: %u", rc);
        return;
    }
}
...
{
    BOOT_LOG_ERR("hw_err_code: %u", hw_err_code);
}
                               ...
                               uint16_t offset, uint16_t max_length)
{
    ...
    case SMP_GATT_ID_NTF_CFG:
        ...
        BOOT_LOG_INF("Value read notification configuration (conidx: %u), config: %u", conidx,
                     env.ntf_cfg);
        rc = GAP_ERR_NO_ERROR;
        break;

    default:
        BOOT_LOG_ERR("Value read to unknown characteristic (conidx: %u), idx: %u", conidx, idx);
    ...
    if (rc != GAP_ERR_NO_ERROR)
    {
        BOOT_LOG_ERR("Failed to confirm value read (conidx: %u), error: %u", conidx, rc);
    }
                              ...
                              uint16_t offset, co_buf_t *p_data)
{
    ...
    case SMP_GATT_ID_VAL:
        ...
        if (rc != GAP_ERR_NO_ERROR)
        {
            BOOT_LOG_ERR("Failed to process SMP request (conidx: %u), error: %u", conidx,
                         rc);
            break;
        }

        BOOT_LOG_INF("Received SMP request (conidx: %u)", conidx);
        break;
    ...
    case SMP_GATT_ID_NTF_CFG:
        ...
        if (rc != GAP_ERR_NO_ERROR)
        {
            BOOT_LOG_ERR("Failed to process notification configuration (conidx: %u), error: "
                         "%u",
                         conidx, rc);
            break;
        }

        BOOT_LOG_INF("Received notification configuration (conidx: %u), config: %u", conidx,
                     env.ntf_cfg);
        break;

    default:
        BOOT_LOG_ERR("Value set to unknown characteristic (conidx: %u), idx: %u", conidx, idx);
        rc = ATT_ERR_REQUEST_NOT_SUPPORTED;
        break;
    }
    ...
    if (rc != GAP_ERR_NO_ERROR)
    {
        BOOT_LOG_ERR("Failed to confirm value set (conidx: %u), error: %u", conidx, rc);
    }
}
...
static uint16_t utils_add_service(void)
{
    ...
    if (rc != GAP_ERR_NO_ERROR)
    {
        BOOT_LOG_ERR("Failed to register gatt server, error: %u", rc);
        return rc;
    }
    ...
    if (rc != GAP_ERR_NO_ERROR)
    {
        BOOT_LOG_ERR("Failed to add gatt service, error: %u", rc);
        return rc;
    }
...
static void on_gapm_name_proc_cmp_cb(uint32_t metainfo, uint16_t status)
{
    ...
    if (rc != GAP_ERR_NO_ERROR)
    {
        BOOT_LOG_ERR("Failed to add service, error: %u", rc);
        return;
    }
    ...
    if (rc != GAP_ERR_NO_ERROR)
    {
        BOOT_LOG_ERR("Failed to create advertising activity, error: %u", rc);
        return;
    }
}
...
static void on_gapm_process_complete(uint32_t metainfo, uint16_t status)
{
    ...
    if (status != GAP_ERR_NO_ERROR)
    {
        BOOT_LOG_ERR("GAPM completion callback failed, error: %u", status);
        return;
    }
...
static void on_ble_enabled(void)
{
    ...
    if (rc != GAP_ERR_NO_ERROR)
    {
        BOOT_LOG_ERR("Failed to configure GAP, error: %u", rc);
        return;
    }
}
...
static int transport_out(struct net_buf *nb)
{
    ...
    while (off < nb->len)
    {
        ...
        {
            BOOT_LOG_ERR("Failed to send notification, error: %u", tx_rc);
    ...
    BOOT_LOG_INF("Sent SMP response notification (conidx: %u)", env.conidx);
...
int boot_ble_init(void)
{
    ...
    if (rc != 0)
    {
        BOOT_LOG_ERR("Failed to initialize SMP transport, error: %i", rc);
        return -1;
    }
